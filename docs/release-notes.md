...
- Chats/WhatsApp: o provider `wwebjs` passa a tratar como envio aceito os erros conhecidos de payload incompleto apos aceite do provider (ex.: leitura de `id`/`_serialized` em `undefined`), evitando falso erro HTTP no endpoint de mensagens e reduzindo nova tentativa que pode duplicar entrega.
- Chats/Tempo real: o endpoint de envio de texto passa a persistir imediatamente a mensagem outbound no ticket apos o provider aceitar o envio, sem depender exclusivamente do eco assincromo do provider; quando o eco chega depois com ID real, o backend reconcilia o registro otimista para evitar duplicidade no chat.
...
- Chats/Contato manual: quando ja existe duplicata historica do mesmo cliente em variante equivalente de numero, o backend passa a preferir o contato que ja sustenta o ticket ativo correto na mesma conexao, em vez de reaproveitar cegamente o contato equivalente mais antigo; isso cobre o caso em que um ticket `open` manual coexistia com um ticket antigo `pending` do mesmo cliente sem o nono digito.
- Chats/Histórico: quando um ticket manual correto ficou vazio porque as mensagens antigas foram persistidas em uma duplicata pendente sem dono/fila do mesmo cliente equivalente, `ListMessagesService` passa a fazer um fallback de leitura e devolver esse histórico no ticket visivel, sem mover registros legados de lugar.
...
- Escopo desta correcao complementar: `backend/src/helpers/BuildEquivalentContactNumberCandidates.ts`, `backend/src/__tests__/unit/Helpers/BuildEquivalentContactNumberCandidates.spec.ts`, `backend/src/services/ContactServices/CreateOrUpdateContactService.ts`, `backend/src/services/ContactServices/__tests__/CreateOrUpdateContactService.spec.ts`, `backend/src/services/TicketServices/FindOrCreateTicketService.ts`, `backend/src/services/TicketServices/__tests__/FindOrCreateTicketService.spec.ts`, `backend/src/services/MessageServices/ListMessagesService.ts`, `backend/src/services/MessageServices/__tests__/ListMessagesService.spec.ts` e `docs/release-notes.md`.
- Validacao local desta correcao complementar: `pnpm exec jest src/handlers/__tests__/handleWhatsappEvents.manualContactEcho.spec.ts src/__tests__/unit/Helpers/BuildEquivalentContactNumberCandidates.spec.ts src/services/ContactServices/__tests__/CreateOrUpdateContactService.spec.ts src/services/TicketServices/__tests__/FindOrCreateTicketService.spec.ts src/services/MessageServices/__tests__/ListMessagesService.spec.ts --runInBand` aprovado em `backend`; `pnpm run build` aprovado em `backend`; diagnosticos do editor sem erros nos arquivos alterados.
