...
exports.DialogTagsService = void 0;
...
let DialogTagsService = class DialogTagsService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    ...
    async updateTag(tenantId, id, input) {
    ...
    async deleteTag(tenantId, id, force = false) {
        const existing = await this.prisma.tag.findFirst({
            where: { id, tenant_id: tenantId },
        });
        if (!existing) {
        ...
        const totalUsage = usage.contacts + usage.conversations + usage.campaigns + usage.automations + usage.dialogs;
    ...
    async ensureDefaultTagRecords(tenantId) {
        const tags = await this.prisma.tag.findMany({
            where: { tenant_id: tenantId },
            select: { name: true },
        });
    ...
    async getUsageCounts(tenantId, tagId) {
        const contacts = await this.prisma.contactTag.count({
            where: { tenant_id: tenantId, tag_id: tagId },
        });
        const conversationRows = await this.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(DISTINCT c.id)::int AS count
        FROM contacts c
        ...
        const campaignRows = await this.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(DISTINCT ctg.campaign_id)::int AS count
        FROM campaign_targets ctg
        JOIN contact_tags ct ON ct.contact_id = ctg.contact_id
        WHERE ct.tenant_id = ${tenantId}
          AND ct.tag_id = ${tagId}`);
        const campaigns = campaignRows[0]?.count ?? 0;
        const automationRows = await this.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*)::int AS count
        FROM automation_actions
        WHERE payload::text ILIKE ${`%${tagId}%`}`);
        const automations = automationRows[0]?.count ?? 0;
        const dialogRows = await this.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*)::int AS count
        FROM "Dialog"
        WHERE tenant_id = ${tenantId}
...
exports.DialogTagsService = DialogTagsService;
exports.DialogTagsService = DialogTagsService = __decorate([
